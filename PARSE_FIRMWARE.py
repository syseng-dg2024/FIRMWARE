...
def parse_nmc_firmware(input_file, output_file):
    ...
    for line in lines[1:]:
        if line.strip():
            values = [v.strip() for v in line.split('\t')]
            if len(values) >= 3:
                ups_type = values[0]      # First column: UPS type (SU, SY, etc.)
                nmc_model = values[1]     # Second column: NMC model (AP9630, etc.)
                firmware_version = values[2]  # Third column: Version
...
if __name__ == "__main__":
    ups_count = parse_ups_firmware('data/raw/APC_UPS_data.txt', 'data/output/APC/UPS/UPS_Firmware_Lookup.json')
    nmc_count = parse_nmc_firmware('data/raw/APC_NMC_data.txt', 'data/output/APC/UPS/NMC_Firmware_Lookup.json')
